...
export default function PostWidget({
  ...
  const addComment = async () => {
    const response = await fetch(
      `https://connectserver.onrender.com/posts/${postId}/addComment`,
      {
        method: "PUT",
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  const deletePost = async (postId, picturePath) => {
    const response = await fetch(
      `https://connectserver.onrender.com/posts/${postId}/delete`,
      {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
              ...
              <FlexBetween>
                <FlexBetween gap="2rem" justifyContent="flex-start">
                  <Avatar
                    ...
                    src={
                      comment?.picturePath
                        ? `https://res.cloudinary.com/ddrbtlpxj/${comment.picturePath}`
                        : defaultAvatar
                    }
                    sx={{ m: "0.5rem 0" }}
                  />
                ...
                {(comment.commentBy === loggedInUserId ||
                  postUserId === loggedInUserId) && (
                  <IconButton
                    onClick={() => {
